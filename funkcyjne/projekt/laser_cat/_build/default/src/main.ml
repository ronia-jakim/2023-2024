...
let tt p = 
  Raylib.draw_rectangle_rec (Pies.return_collider p) Raylib.Color.green
...
let rec loop (pause, back_img, player, pies_list) =
  ...
  | false ->
    ...
    List.iter tt pies_list;

    Raylib.draw_rectangle_rec (Kot.return_collider player) Raylib.Color.red;
