...
\begin{example}
  ...
  \item Uogólniając obserwację wyżej, w dowolnej kategorii $\mathbf{C}$ obiektowi $X$ możemy przypisać funktor 
    ...
    \begin{center}\begin{tikzcd}[column sep = large]
      ...
      X \arrow[r, squiggly, "h_X(f)" below] & X
...
\begin{definition}[Kategoria funktorów i funktory reprezentowalne]
  ...
  Funktor $F\in\hat{\mathbf{C}}$ jest \buff{reprezentowalny}, jeśli $F\cong h_X$ dla pewnego $X\in Ob\mathbf{C}$. Takie $X$ jest jedyne z dokładnością do izomorfizmu. Dla morfizmu $X\xrightarrow{\phi} X'$ w $\mathbf{C}$ określamy morfizm $h_\phi:h_X\to h_{X'}$ w $\mathbf{\hat{C}}$.
