...
\item Niech $\mathbf{A}$ i $\mathbf{B}$ będą kategoriami abelowymi, a $T:\mathbf{A}\to \mathbf{B}$ addytywnym funktorem między tymi kategoriami.
  ...
  Weźmy obiekt $A\in\ob\mathbf{A}$ i dwie jego projektywne rezolwenty \begin{tikzcd}P^*\arrow[r, yshift=-1mm, "f" below]&Q^*\arrow[l, yshift=1mm, "g" above]\end{tikzcd} takie, że $fg$ i $gf$ są oba homotopijnie równoważne z identycznością. Wtedy oczywiście $T(P^*)$ i $T(Q^*)$ są kompleksami:
  $$T(d)T(d)=T(dd)=T(0)=0$$
